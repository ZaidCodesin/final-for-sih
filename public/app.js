...
/* ProtecT frontend — role-aware, privacy-first vanilla SPA */
...
/* On-device AI voice engine (free, offline, private) — chunked Whisper via a worker.
...
function voiceOnWorkerMessage(event){const msg=event.data||{};if(msg.type==='diag'){if(msg.session!==jrVoiceSession)return;setVoiceUi(true,`Voice check: ${msg.runtime} · ${msg.threaded} · model file HTTP ${msg.encoder}`);return;}if(msg.type==='progress'){if(msg.session!==jrVoiceSession)return;const pct=typeof msg.progress==='number'&&msg.progress<=1?Math.round(msg.progress*100):(msg.loaded&&msg.total?Math.round(msg.loaded/msg.total*100):null);setVoiceUi(true,pct==null?`Loading private voice AI… ${msg.file||''}`.trim():`Loading private voice AI… ${pct}%`);return;}if(msg.type==='ready'){if(msg.session!==jrVoiceSession)return;jrVoiceWorkerReady=true;jrVoiceLoading=false;if(!jrListening)return;voiceBeginCapture();return;}if(msg.type==='transcript'){const pending=jrVoicePending.get(msg.id);jrVoicePending.delete(msg.id);if(!jrListening||!pending||pending.session!==jrVoiceSession)return;voiceHandleTranscript(pending.session,msg.text,pending.seconds);return;}if(msg.type==='error'&&msg.id==null){if(msg.session!==jrVoiceSession)return;jrVoiceLoading=false;jrVoiceAiError=String(msg.message||'model failed');voiceUseBrowserFallback(`On-device AI failed to start (${jrVoiceAiError}). Using browser voice typing.`);return;}if(msg.type==='error'){jrVoicePending.delete(msg.id);if(jrListening)setVoiceUi(true,'Heard something unclear · still listening');}}
async function voiceLoadModel(){if(jrVoiceWorkerReady)return true;if(jrVoiceLoading)return 'loading';const worker=voiceEnsureWorker();if(!worker)return false;jrVoiceLoading=true;setVoiceUi(true,'Loading private voice AI (one-time download)…');try{worker.postMessage({type:'load',session:jrVoiceSession});}catch{jrVoiceLoading=false;return false;}return 'loading';}
function voiceMicErrorMessage(error){const name=(error&&(error.name||error.code))||'';if(name==='NotAllowedError'||name==='SecurityError')return 'Microphone blocked. Allow microphone access for this site, then tap Speak again — or continue by writing.';if(name==='NotFoundError'||name==='OverconstrainedError')return 'No microphone found. Connect a microphone and tap Speak again — or continue by writing.';if(name==='NotReadableError'||name==='AbortError')return 'Microphone is busy (another app may be using it). Close it and tap Speak again.';if(!window.isSecureContext)return 'Microphone needs a secure page. Open this app at http://localhost:4400 (not a network address over plain http), or continue by writing.';return 'Could not open the microphone. Check the browser permission and tap Speak again.';}
function setVoiceUi(active,status=''){const button=$('#jr-mic');if(!button)return;button.classList.toggle('recording',active);button.setAttribute('aria-pressed',String(active));button.innerHTML=active?'<span aria-hidden="true">■</span> Stop speaking':'<span aria-hidden="true">●</span> Speak privately';const label=$('#jr-mic-status');if(label)label.textContent=status;}
...
function commanderTrendMarkup(rows){if(!rows.length)return emptyMarkup('No trend yet','Organizational records will build the trend.');const totalHigh=rows.reduce((n,r)=>n+(r.high_load_units||0),0),totalMonitor=rows.reduce((n,r)=>n+(r.monitor_units||0),0);return `<canvas id="cmd-trend-chart" width="1000" height="280" role="img" aria-label="Organizational pressure trend across ${rows.length} days: ${totalHigh} high-load unit-days and ${totalMonitor} monitor unit-days"></canvas><div class="trend-legend" aria-hidden="true"><span><i class="high"></i>High load units</span><span><i class="monitor"></i>Monitor units</span><span><i class="overtime"></i>Total overtime hours</span></div><p class="source-note">Last 30 days, aggregated across all units. First day: ${rows[0].high_load_units||0} high · ${rows[0].monitor_units||0} monitor. Latest: ${rows[rows.length-1].high_load_units||0} high · ${rows[rows.length-1].monitor_units||0} monitor.</p>`;}

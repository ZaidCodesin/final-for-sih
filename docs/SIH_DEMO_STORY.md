# ProtecT connected SIH demo story
...
1. **Explore the Personnel Demo.** Point out the â€œDemo environmentâ€ label, then sign in as Rahul.
2. **Start from personal value.** The home screen opens with â€œYour private wellbeing space,â€ not an employer score.
...
8. **Explain the personal pattern.** Open â€œSomething worth noticingâ€ and its evidence, such as lower sleep alongside longer-duty days. State that this is an observed association, not causation.
9. **Open Work Context.** Show Rahul's official duty/overtime/leave/recovery records, their source and update time, why they are used, and the tracked â€œReport incorrect recordâ€ action.
...
11. **Show the locked journal.** The request visibly says â€œPRIVATE JOURNAL â€” LOCKED â€” not shared.â€ Submit and note the received case ID/status.
12. **Switch to the Welfare Officer Demo.** The same new case appears in the single Support Queue with its explicit source: â€œPersonnel requested support.â€ No second risk roster competes with it.
...
15. **Return to Personnel.** Rahul now sees â€œContactedâ€ or the scheduled follow-up in Privacy & Support. This closes the request-to-response loop.
16. **Switch to the Commander Demo.** Lead with â€œConditions, never case files.â€ The Commander cannot see Rahul, the support case, assessments, or Journal.
17. **Open Units and Early Indicators.** Show the same unit metrics in Overview, heatmap, unit details, and the seven-day indicator. Expand â€œWhy?â€ to show overtime, recovery, leave, deployment, incidents, consented voluntary coverage, and data-quality limitations.
...
- Ask whether the model is clinically validated; answer noâ€”the demo uses a transparent simulated baseline and documents the validation path required for production.

'use strict';
/* ProtecT â€” deterministic, story-driven demo data.
...
function main() {
  ...
  console.log('Seed complete â†’ ' + counts);
}
...
function seedPerson(pid, personNumber, family, profile, unit, { insertHR, insertCI, insertAS, insertJE }) {
  ...
  // Content is generated locally from templates â€” private, simulated only.
...
function journalEntryTemplate(profile, pressure, daysAgoCount) {
  ...
  const closers = tough ? [
    ...
    'Aaj gratitude likhna chahta hoon â€” sehat, family, aur duty ka maan.',
...
function seedSupportCases(personnelByForceId, welfareId) {
  const cases = [
    ...
    { forceId: 'CRPF100041', source: 'self_request', reason: 'personal_difficulty', priority: 'Routine',
      ...
      nextAction: 'None â€” person may reopen support', createdAgo: 50, responseAgo: 48, contactAgo: 31,

'use strict';
/* ProtecT voice model fetcher â€” free, offline-first on-device AI speech-to-text.
 *
...
async function main() {
  ...
  for (const file of ['quant_config.json', 'quantize_config.json']) {
    try {
      ...
      console.log(`  Â· models/${model}/${file} not published â€” skipped`);
    }
  }
  await fs.promises.writeFile(path.join(root, 'manifest.json'), JSON.stringify(manifest, null, 2));
  console.log(`\nVoice assets ready â†’ public/vendor/voice (${(bytes / 1048576).toFixed(0)} MB total, model ${model}).`);
